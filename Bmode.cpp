...
int main(){
    ...
    //Creates a linked list iterator variable for the scanlines
    BmodeClass *currentScanline = scanlineHead;

    //Creates a linked list iterator variable for the dataBuffers
    dataBuffer *dataIterator = dataHead;

    //Generates the scanline linked list
    for(int i = 1 ; i < parameters->getNumScanline(); ++i){
        //Goes to the next entry of the dataBuffer linked list
        dataIterator = dataIterator->next;

        //Creates a new BmodeClass entry for the BmodeClass linked list
        currentScanline->next = new BmodeClass(parameters, dataIterator, i);

        //Sets the current iterator the new entry
